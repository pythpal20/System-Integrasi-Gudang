...
<script>
    $(document).ready(() => {
            ...
            if (sts == 'plus' && role == '2') { //jika penambahan barang
                if (iStatus == '1' || iStatus == '2') {
                    return [
                        ...
                        '<button class="btn btn-sm btn-primary approve" id="approve" rel="tooltip" title="Konfirm Penambahan Barang" data-code="' + value + '" data-qty="' + row.qty + '" disabled><i class="fas fa-fw fa-check"></i></button>'
                    ]
                } else {
                    return [
                        '<button class="btn btn-sm btn-success lihats" rel="tooltip" title="View Detail" data-code="' + value + '"><i class="fas fa-fw fa-eye"></i></button> ' +
                ...
                if (iStatus == '0') {
                    ...
                    return [
                        '<button class="btn btn-sm btn-success lihats" rel="tooltip" title="View Detail" data-code="' + value + '"><i class="fas fa-fw fa-eye"></i></button> ' +
                        '<button class="btn btn-sm btn-primary approve" id="approve" rel="tooltip" title="Konfirm Pengurangan Barang" data-code="' + value + '" data-qty="' + row.qty + '" disabled><i class="fas fa-fw fa-check"></i></button>'
                    ]
                }
                ...
                if (dest == 'g75' && jol == 'g75') { //jika yang menerima barang adalah gudang 75 maka yang confirm adalah admin gudang 75
                    if (iStatus == '0') {
                        ...
                        return [
                            '<button class="btn btn-sm btn-success lihats" rel="tooltip" title="View Detail" data-code="' + value + '"><i class="fas fa-fw fa-eye"></i></button> ' +
                            '<button class="btn btn-sm btn-primary approve" id="approve" rel="tooltip" title="Konfirm Terima Barang" data-code="' + value + '" data-qty="' + row.qty + '" disabled><i class="fas fa-fw fa-check"></i></button>'
                        ]
                    }
        ...
        $('body').on('click', '#tb_mutasi .approve', function() {
            var id = $(this).data('code');
            window.open('<?= base_url("warehouse/apm/"); ?>' + id, 'blank');
        });
